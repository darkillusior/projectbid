...
function Post({ postsData, user }) {
  ...
  let bidusertrue = post.bid.some(
    (bid) => bid.user.toString() === (user ? user._id : null)
  );
  const [bidtrue, setbidtrue] = useState(bidusertrue);
  let ideatruserue = post.idea.some(
    (bid) => bid.user.toString() === (user ? user._id : null)
  );
  ...
  }, [post]); // Add post as a dependency to the useEffect
    ...
    <>
      <main className={styles.bg}>
        ...
        <div className={" h-auto sm:mx-5 mt-10 sm:mt-20 flex flex-col"}>
          <div className="flex flex-col  justify-between ">
            <div className="flex justify-between sm:items-center  ">
              <div className={" cursor-pointer " + styles.container}>
                ...
                <h1 className="text-white text-2xl font-bold py-2 px-5">
                  {" "}
                  min: &#8377;{post.bidprice}
                </h1>
              </div>
            ...
            <h1
              className={
                " mt-7 text-center backdrop-blur-3xl font-bold text-4xl sm:text-5xl  text-white  " +
                styles.heading2
              }
            >
          ...
          >
            ...
            <div className="rounded-md flex flex-col items-start gap-5 border-t-0 sm:w-1/2 mt-10">
              <div className="m-2 flex items-center  ">
                <img
                  className={
                    "h-24 w-24 m-2  rounded-full object-cover" + styles.userPic
                  }
                  src={post.userimg}
                ></img>
          ...
          <br />
          {/* <hr /> */}
          {user && (user.role === "root" || post.user === user._id) && (
            <Modal idea={idea} />
          )}
                  ...
                  <tr className="bg-slate-900/50 border-slate-800 border hover:bg-slate-800/50 cursor-pointer ">
                    ...
                    {user && (user.role === "root" || bids.user === user._id) && (
                      <td className="w-2/4 text-xl p-2 text-center font-serif text-red-600">
                        <button
                          onClick={() =>
                            deletePost(post._id, setBids, user._id)
                          }
                        >
                          delete
                        </button>
